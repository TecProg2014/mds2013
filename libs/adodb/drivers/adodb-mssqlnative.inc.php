...
class ADODB_mssqlnative extends ADOConnection {
    ...
    function _query($sql, $inputarr = false) {
        ...
        if ($this->debug)
            error_log("<hr>running query: " . var_export($sql, true) . "<hr>input array: " . var_export($inputarr, true) . "<hr>result: " . var_export($rez, true)); //"<hr>connection: ".serialize($this->_connectionID)
...
//fix for returning true on anything besides select statements
        ...
        if (stripos($sql, 'SELECT') !== 0 && $rez !== false) {
            if ($this->debug)
                error_log(" isn't a select query, returning boolean true");
            return true;
        }
        //end fix
        if (!$rez)
